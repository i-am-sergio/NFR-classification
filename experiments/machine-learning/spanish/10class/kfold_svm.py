# -*- coding: utf-8 -*-
"""
kfold_svm.py (Clasificación de 10 Clases)

Este script evalúa un clasificador Support Vector Machine (SVM) con un kernel RBF
en una tarea de clasificación de 10 clases, excluyendo 'F' y 'PO', utilizando
una validación cruzada estratificada de 10 pliegues.
...
5.  DENTRO DEL BUCLE (para cada pliegue):
    a. Escala las características (StandardScaler), ajustando solo en el train set.
    b. Entrena un nuevo modelo SVM en 9 pliegues escalados.
    c. Evalúa en el pliegue restante escalado.
    d. Muestra un reporte de rendimiento detallado para el pliegue actual.
6.  FUERA DEL BUCLE:
    a. Calcula y muestra métricas promedio y la matriz de confusión agregada.
"""

print("--- Iniciando Pipeline: 10-Fold CV con Support Vector Machine (SVM) (Clasificación de 10 Clases) ---")
...
for fold, (train_index, test_index) in enumerate(skf.split(X, y_numeric)):
    ...
    # Escalado de Características (esencial para SVM)
...
# ----------------------------------------
print(f"\n{'='*30} RESULTADOS FINALES AGREGADOS (SVM - 10 CLASES) {'='*30}")
...
plt.title('Matriz de Confusión Agregada (SVM - 10 Clases, 10-Fold CV)')
