# -*- coding: utf-8 -*-
"""
kfold_xgboost.py (Clasificación de 10 Clases)

Este script evalúa un clasificador XGBoost en una tarea de clasificación de
10 clases, excluyendo 'F' y 'PO', utilizando una validación cruzada
estratificada de 10 pliegues.
...
5.  DENTRO DEL BUCLE (para cada pliegue):
    a. Entrena un nuevo modelo XGBoost en 9 pliegues (usando GPU y early stopping).
...
Nota: XGBoost no requiere escalado de características.
"""

print("--- Iniciando Pipeline: 10-Fold CV con XGBoost (Clasificación de 10 Clases) ---")
...
print(f"XGBoost se ejecutará en el dispositivo: {device.upper()}")
...
for fold, (train_index, test_index) in enumerate(skf.split(X, y_numeric)):
    ...
    # Para clasificación multiclase, el objective es 'multi:softmax'
...
# ----------------------------------------
print(f"\n{'='*30} RESULTADOS FINALES AGREGADOS (XGBOOST - 10 CLASES) {'='*30}")
...
plt.title('Matriz de Confusión Agregada (XGBoost - 10 Clases, 10-Fold CV)')
